...
vocplot <- ggplot(dat_long, aes(x = elapsed.time, y = Flux, colour = group)) + 
  ...
  labs(x = "Time after slurry application (hours)", y = ("mg/m2")) +
...
vocflux <- ggplot(sum.voc, aes(x = elapsed.time, y = Sum_Flux, fill = Group)) +
  ...
  labs(
    ...
    y = expression(paste(" mass (mg ", m^-2, ")"))
  ) + 
...
####################################################################
####################################################################
OTV <- read_excel("VOC_OTV.xlsx")
# Ensure OTV has the correct column names
OTV <- OTV %>%
  rename(
    methanol = `Methanol`,
...
# Create the plot with thinner lines and points
all <- ggplot(oav_long, aes(x = elapsed.time, y = value, colour = group)) + 
  geom_point(size = 0.4) +  # Adjust point size
  geom_line(aes(group = valve), linewidth = 0.5) +  
  facet_wrap(~ compound, scales = "free_y") + 
  labs(x = "Time after slurry application (hours)", y = "oav")+
  theme_minimal(); all
#ggsave("Figure/endhours/OAV.png", plot = vocplot)
...
#ggsave("Figure/endhours/OAV group.png", plot = vocflux)
...
# Define time bins
oav_long <- oav_long%>%
  mutate(Time_Bin = cut(elapsed.time, breaks = c(seq(0, 120, by = 10), 124), include.lowest = TRUE, right = FALSE))

oav_long <- oav_long %>%
  group_by(Time_Bin, compound) %>%
  reframe(
    Total_Value = sum(value, na.rm = TRUE),
    treatment = unique(treatment),  # Keeps all unique treatments in the group
    Group = unique(Group),          # Keeps all unique values in 'Group' for each group
    group = unique(group)           # Keeps all unique values in 'group' for each group
  )

oav_long<- oav_long %>%
group_by(Time_Bin) %>%  # Regroup by hour_group and treatment# Group by hour_group, Group, and treatment
  mutate(Percentage = (Total_Value / sum(Total_Value)) * 100)%>%  # Calculate percentage for each hour_group
ungroup()


ggplot(oav_long, aes(x = Time_Bin, y = Percentage, fill = Group)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(
    title = "Percentage of Total Value for Each Compound by Time Bin",
    x = "Time Bin",
    y = "Percentage",
    fill = "Compound"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +facet_wrap(~treatment)  # Facet the plot by 'treatment'
