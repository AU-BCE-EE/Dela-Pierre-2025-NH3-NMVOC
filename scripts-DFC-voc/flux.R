...
weather <- read_csv("../input-DFC/Temp.csv") #find this in Flavia_VOC_DFC_data folder

# Import VOC data, the 30s background corrected averages created in the VOC concentrations and OAV.R
dat <- read_delim("../output-DFC/voc.30s.corrected.txt", 
                  delim = ",", escape_double = FALSE, trim_ws = TRUE)
vsc_ini <- read_csv("../output-DFC/vsc.first.minutes.txt") #file created in the VOC concentrations and OAV script, these are the initial concentrations for VSC (first 8 minutes)
...
group_emissions <- final_emissions %>%
  ...
  # Calculate total emission for each group (excluding H2S)
  mutate(
    total_emission = rowSums(
      across(all_of(grep("^cum\\.", names(.), value = TRUE)[grep("^cum\\.", names(.), value = TRUE) != "cum.H2S"])), 
      na.rm = TRUE
    )
  )
...
#save flux
write_csv(final_dat, "../output-DFC/flux_voc_dfc.csv")
#save cumulative emissions

write_csv(final_emissions, "../output-DFC/cum.voc.emis.csv")
